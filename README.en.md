...
- Viewer ClickOnce publish uses a `.NET 8 SDK + Visual Studio ClickOnce` compatibility workaround: nested agent builds are isolated from outer publish globals, and legacy VS ClickOnce targets are suppressed in `ScreensView.Viewer/Directory.Build.targets`.
