...
9. Карточки в основной сетке можно выбирать кликом. Используйте **Ctrl+клик**, чтобы добавить или убрать карточку, **Shift+клик**, чтобы выбрать диапазон, и **Ctrl+A**, чтобы выбрать все карточки. Правый клик по выбранной карточке открывает контекстное меню для выбранных компьютеров; **Открыть** и **Редактировать** остаются действиями для карточки, по которой открыто меню, а применимые действия вроде **Запустить LLM сейчас**, **Пинг**, удалённого доступа, управления питанием и **Удалить** выполняются для всего выбора.
...
2. При первом запуске выберите модель и нажмите **Скачать**. По умолчанию выбран совместимый `LLaVA v1.5 7B`. В списке также доступны `Gemma 4 E2B`, `Qwen3.5-0.8B`, `Qwen3.5-9B`, `Qwopus3.5-9B Coder` и другие GGUF-варианты из настроек.
...
#### Добавление и редактирование моделей (`models.json`)

Список моделей в настройках управляется обычным JSON-файлом `%AppData%\ScreensView\models.json`. При первом запуске Viewer создаёт его со встроенными моделями — это готовый шаблон для копирования и правки. Чтобы изменения применились, перезапустите Viewer.

Файл — это JSON-массив; у каждой записи такие поля:
...
- `ProjectorFileName` / `ProjectorDownloadUrl` необязательны, но указываются только вместе либо вообще отсутствуют (vision-модели нужен projector mmproj; у текстовой модели его нет).
- Файл **сливается** со встроенным списком: запись с `Id` встроенной модели переопределяет её (удобно сменить название или зеркало URL), а новый `Id` добавляется в список. Новые встроенные модели из будущих версий Viewer всё равно появляются автоматически. Встроенную модель нельзя удалить через файл — только переопределить.
- Правила проверки — невалидные записи отбрасываются, остальные сохраняются: `FileName`/`ProjectorFileName` должны быть именем файла без путей (нет разделителей каталога, нет `..`, не абсолютный путь); `DownloadUrl`/`ProjectorDownloadUrl` — абсолютные `http`/`https` URL; две модели не могут делить одно локальное имя файла.
- Если файл отсутствует, пуст или повреждён, Viewer откатывается на встроенный каталог и всегда запускается. Повреждённый файл не перезаписывается, чтобы его можно было поправить.
...
- Чекбокс **Сворачивать в трей при закрытии** в секции `Общие` — при закрытии окна Viewer сворачивается в системный трей вместо завершения. Значок в трее предоставляет контекстное меню: показать/скрыть окно, открыть настройки, выйти.
