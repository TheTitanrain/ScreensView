...
4. При необходимости включить **Автозапуск** в верхней command bar, чтобы Viewer запускался вместе с Windows
...
Выбрать компьютер → **Управление компьютерами** → остановит службу, заменит payload, затем снова запустит службу. Кнопка **Обновить агентов** в верхней command bar обновляет все машины параллельно, сохраняя выбор `modern`/`legacy` по ОС.
...
- Слайдер **Интервал (сек)** в верхней command bar сохраняет последнее выбранное значение в `RefreshIntervalSeconds` и восстанавливает его при следующем запуске Viewer.
- Чекбокс **Автозапуск** в верхней command bar включает или выключает запуск Viewer при входе в Windows.
