...
3. В диалоге **Добавить компьютер** заполнить секции **Компьютер**, **Подключение** и при необходимости **Описание экрана**: имя, хост, порт и API-ключ.
...
- Описание должно задавать общий тип экрана и его стабильную визуальную структуру: расположение крупных блоков, колонок, цветовых зон, таймлайнов, таблиц или плиток.
- Не включайте в описание мелкие и постоянно меняющиеся детали: точные времена, номера заявок, госномера, имена сотрудников, комментарии, модели автомобилей.
- При LLM-сравнении Viewer ориентируется в первую очередь на общий layout экрана, а не на точное совпадение всех динамических полей или на то, заполнены строки данными или временно пусты.
