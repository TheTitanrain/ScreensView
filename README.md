...
3. Если выбран backend `CUDA (NVIDIA)`, Viewer скачивает и сам `llama-server`, и отдельный архив с CUDA runtime DLL. Если статус backend показывает `Не скачан` или `Скачан не полностью`, нажмите **Скачать бэкенд** в настройках, чтобы восстановить локальные бинарники.
...
Во время проверки экрана `viewer.log` теперь также пишет итог по каждому компьютеру: имя компьютера, время распознавания в миллисекундах (`elapsedMs`), итог (`match` / `mismatch` / `error`), был ли сохранён результат (`stored`), исходное описание экрана и explanation от модели.
